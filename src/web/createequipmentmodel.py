...
async def createequipmentmodel(request: Request,db: Session=Depends(get_db),msg:str=None,sn:str=None):
    ...
    if await form.is_valid(db=db):
        try:
            ...
            print(f'{e}')
            raise HTTPException(status_code=302, detail="Not authorized", headers={"Location": "/login"}) from e
    return templates.TemplateResponse("registerequipmentmodel.html",{"request":request,"manuf":manuf,"equipmenttypenew":equipmenttypenew,"errors": form.__dict__['errors'],"user":current_user})
