...
async def actionstatusnew(request: Request,db: Session=Depends(get_db),msg:str=None,sn:str=None,status:str=Form(),recieve_note:str=Form(),billid:str=Form(),billamount:int=Form()):
    ...
    current_user: User = get_current_user_from_token(token=param, db=db)
    status=status
    recieve_note=recieve_note
    try:
        if status == "RR":
           activity=QueryModelData(modeltable=EquipmentActivity,db=db,cols={"sn":sn,"maintaince_status":None,"next_activity":"T"}).first()
           if activity.place_of_maintaince=="L":
                localactivity= updateactivity(next_activity="F", maintaince_status=status,recieve_by=current_user.staffno,recieve_note=recieve_note)
                UpdateModelData(modeltable=EquipmentActivity,col_id={"activityid":activity.activityid},updatecols=localactivity,db=db)
                activitycreate=EquipmentActivityCreate(registerid=localactivity.registerid,create_by=current_user.staffno,activity_desc="Waiting for return back to Department",next_activity="T",place_of_maintaince="L",activity_status="WFR")
                CreateModelData(modeltable=EquipmentActivity,db=db,modelcreate=activitycreate)
                return RedirectResponse('/'+'?sn='+sn, status_code=303)
           else:
                sentactivity= updateactivitycompany(next_activity="F", maintaince_status=status,recieve_by=current_user.staffno,recieve_note=recieve_note,billid=billid,billamount=billamount)
                UpdateModelData(modeltable=EquipmentActivity,col_id={"activityid":activity.activityid},updatecols=sentactivity,db=db)
                activitycreate=EquipmentActivityCreate(registerid=localactivity.registerid,create_by=current_user.staffno,activity_desc="Waiting for return back to Department",next_activity="T",place_of_maintaince="L",activity_status="WFR")
                CreateModelData(modeltable=EquipmentActivity,db=db,modelcreate=activitycreate)
                return RedirectResponse('/'+'?sn='+sn, status_code=303)
        else:
            ...
            return templates.TemplateResponse("sendtocompany.html",{"request": request})
    return templates.TemplateResponse("sendtocompany.html",{"request": request})
